...
namespace GenshinImpactMovementSystem
{
    public class PlayerMovementState : IState {
        ...
        private void Move() {
            ...
            float movementeSpeed = GetMovementSpeed();

            Vector3 currentPlayerHorizontalVelocity = GetPlayerHorizontalVelocity();
            // Recordar que es mucho más optimo multiplicar números con números y luego vectores que mult. num * vec * num, por ejemplo.
            stateMachine.Player.Rigidbody.AddForce(targetRotationDirection * movementeSpeed - currentPlayerHorizontalVelocity, ForceMode.VelocityChange);
        }
        ...
        protected void RotateTowardsTargetRotation() {
            ...
            float smoothYAngle = Mathf.SmoothDampAngle(currentYAngle, stateMachine.ReusableData.CurrentTargetRotation.y, ref stateMachine.ReusableData.DampedTargetRotationCurrentVelocity.y, 
        ...
        protected Vector3 GetTargetRotationDirection(float targetAngle) {
            // Las multiplicacion de un Quaternion * Vector tiene que ser siempre en ese orden y no a la inversa.
        ...
        protected bool IsMovingHorizontally(float minimalMagnitude = 0.1f) {
            Vector3 playerHorizontalVelocity = GetPlayerHorizontalVelocity();
            Vector2 playerHorizontalMovement = new Vector2(playerHorizontalVelocity.x, playerHorizontalVelocity.z);

            return playerHorizontalMovement.magnitude > minimalMagnitude;
        }
        protected bool IsMovingUp(float minimalVelocity = 0.1f) {
            return GetPlayerVerticalVelocity().y > minimalVelocity;
        }
        protected bool IsMovingDown(float minimalVelocity = 0.1f) {
            return GetPlayerVerticalVelocity().y < -minimalVelocity;
        }
